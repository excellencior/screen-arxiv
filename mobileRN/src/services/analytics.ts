...
export function computeOverviewStats(movies: any[], shows: any[]): OverviewStats {
    ...
    for (const show of shows) {
        ...
        if (totalSeasonsWatched === 0 && (show.status === 'WATCHED' || show.status === 'DONE')) {
            totalSeasonsWatched += show.number_of_seasons || (Array.isArray(show.seasons) ? show.seasons.length : (typeof show.seasons === 'number' ? show.seasons : 1));
        }
    }
